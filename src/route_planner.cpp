...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node)
{
    ...
    while (!(current_node->parent==nullptr)) //current_node->x != this->start_node->x && current_node->y != this->start_node->y)
    {
        path_found.push_back(*current_node);
        distance += current_node->distance(*(current_node->parent));
        current_node = current_node->parent;
    }
    path_found.push_back(*current_node); //add the starting node that has not parent
...
void RoutePlanner::AStarSearch()
{
    ...
    this->start_node->visited = true; //set the starting node as visited
